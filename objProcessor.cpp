...
void objPcs::load_specular(TGAProcessor *spec) {
	specularmap_ = spec;
...
float objPcs::specular(vector2f uv) {
	vector2i uvn(uv[0] * specularmap_->get_width(), uv[1] * specularmap_->get_height());
	return specularmap_->get(uvn[0], uvn[1])[0] / 1.f;
}
